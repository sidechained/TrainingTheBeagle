...
- Note: Tried Fredrik's approach but did not do this `sudo apt-get install firmware-ralink firmware-realtek` as I guessed I'd installed this previously (which turned out to be correct). In my /etc/interfaces I enabled both ethernet and wifi at the same time and it was still possible to ssh in via ethernet, however it took some time for ssh to become available (i guess whilst the wifi dongle was being initialised).
...
Listening on LPF/wlan0/e8:4e:06:14:47:10
Sending on   LPF/wlan0/e8:4e:06:14:47:10
Sending on   Socket/fallback
```

`$ sudo sudo ifup wlan0`  
produces this
```
Internet Systems Consortium DHCP Client 4.2.2
Copyright 2004-2011 Internet Systems Consortium.
